...
int main(int argc, char** argv)
{
	...
	{
		unsigned int rand_seed = basic_arg_parser.Convert
		(
			"main-seed",
			[](const decltype(basic_arg_parser)::string_type& val_) -> unsigned int
			{
				if (val_.empty())
				{
				...
				{
					auto val_lower = EmuCore::to_lower<decltype(basic_arg_parser)::string_type>()(val_);
					if (EmuCore::TMP::is_one_of<false>(val_, "random", "rand", "no", "none", "null", "default", "def", "time(0)", "time", "time()"))
					{
						return static_cast<unsigned int>(time(0));
					}
					else
					{
						try
						{
							long long as_int = std::stoll(val_);
	...
	{
		using example_mat_type = EmuMath::Matrix<4, 4, double, true>;
		constexpr auto example_mat = example_mat_type
		(
			12, -26, 23.14, 26.61,
			29, -17, 15,    -26.61,
			41, 0,   29,    512.61,
...
#pragma region PRE_TEST_BODY
	...
	//*
	{
		...
		if (speed_test_count > 0)
		{
			...
			(
				basic_arg_parser.ToInt<std::size_t>("noise-x"),
				basic_arg_parser.ToInt<std::size_t>("noise-y"),
				basic_arg_parser.ToInt<std::size_t>("noise-z")
			);
