...
[[nodiscard]] constexpr inline EmuMath::NewVector<OutSize_, OutT_> _some_new_vec_func_test_helper(const EmuMath::NewVector<OutSize_, OutT_>& starting_out_, Args_&&...args_)
{
	...
	EmuMath::Helpers::vector_negate_range_no_copy<IndexArgs_...>(out_, std::forward<Args_>(args_)...);
	return out_;
}
...
{
	EmuMath::NewVector<OutSize_, OutT_> out_(starting_out_);
	EmuMath::Helpers::vector_negate_range_no_copy<2, 3>(out_, std::forward<Args_>(args_)...);
	return out_;
}
...
int main()
{
	...
	constexpr auto ct_theoretical_0_ = EmuMath::Helpers::vector_get_theoretical<0>(newvector_a_);
	constexpr auto ct_theoretical_1_ = EmuMath::Helpers::vector_get_theoretical<1>(newvector_a_);
	constexpr auto ct_theoretical_2_ = EmuMath::Helpers::vector_get_theoretical<2>(newvector_a_);
	constexpr auto ct_theoretical_3_ = EmuMath::Helpers::vector_get_theoretical<3>(newvector_a_);
	constexpr auto ct_theoretical_4_ = EmuMath::Helpers::vector_get_theoretical<4>(newvector_a_);

	auto& rt_theoretical_0_ = EmuMath::Helpers::vector_get_theoretical<0>(newvector_b_);
	auto& rt_theoretical_1_ = EmuMath::Helpers::vector_get_theoretical<1>(newvector_b_);
	auto& rt_theoretical_2_ = EmuMath::Helpers::vector_get_theoretical<2>(newvector_b_);
	auto& rt_theoretical_3_ = EmuMath::Helpers::vector_get_theoretical<3>(newvector_b_);
	auto  rt_theoretical_4_ = EmuMath::Helpers::vector_get_theoretical<4>(newvector_b_);
	...
	std::cout << "---\n";
	//EmuMath::Helpers::vector_set(newvector_c_, newvector_b_);
	newvector_c_ = decltype(newvector_c_)(newvector_b_);
	std::cout << newvector_a_ << " | " << newvector_b_ << " | " << newvector_c_ << "\n";
	//EmuMath::Helpers::vector_set(newvector_c_, newvector_a_);
	newvector_c_.Set(newvector_a_);
	std::cout << newvector_a_ << " | " << newvector_b_ << " | " << newvector_c_ << "\n";
	//EmuMath::Helpers::vector_set(newvector_b_, newvector_a_);
	newvector_b_.Set(newvector_a_);
	std::cout << newvector_a_ << " | " << newvector_b_ << " | " << newvector_c_ << "\n";
	EmuMath::NewVector<5, float> some_vector_(1.0f, 10.0f, 100.0f, 1000.0f, 6.0f);

	newvector_b_.Set(some_vector_);
	std::cout << newvector_a_ << " | " << newvector_b_ << " | " << newvector_c_ << " | " << some_vector_ << "\n";
	some_vector_.at<2>() = 3.0f;
	std::cout << newvector_a_ << " | " << newvector_b_ << " | " << newvector_c_ << " | " << some_vector_ << "\n";
	some_vector_.Set(newvector_a_);
	//EmuMath::Helpers::vector_set(some_vector_, newvector_a_);
	std::cout << newvector_a_ << " | " << newvector_b_ << " | " << newvector_c_ << " | " << some_vector_ << "\n";
	some_vector_.at<1>() = 1.234f;
	std::cout << newvector_a_ << " | " << newvector_b_ << " | " << newvector_c_ << " | " << some_vector_ << "\n";
	//EmuMath::Helpers::vector_set(some_vector_, std::move(newvector_b_));
	//some_vector_.Set(std::move(newvector_b_));
	newvector_b_ = decltype(newvector_b_)(some_vector_);
	std::cout << newvector_a_ << " | " << newvector_b_ << " | " << newvector_c_ << " | " << some_vector_ << "\n";
	new_num_ = 33.5f;
	//EmuMath::Helpers::vector_set_all(newvector_b_, new_num_);
	newvector_b_.SetAll(new_num_);
	std::cout << newvector_a_ << " | " << newvector_b_ << " | " << newvector_c_ << " | " << some_vector_ << "\n";
	//EmuMath::Helpers::vector_set_all<1, 3>(some_vector_, new_num_);
	some_vector_.SetAll(new_num_);
	std::cout << newvector_a_ << " | " << newvector_b_ << " | " << newvector_c_ << " | " << some_vector_ << "\n";
	new_num_ = 1337.0f;
	std::cout << newvector_a_ << " | " << newvector_b_ << " | " << newvector_c_ << " | " << some_vector_ << "\n";
	...
	memcpy(some_random_arr_.data(), EmuMath::Helpers::vector_data(some_vector_), sizeof(float) * 4);
	std::cout << "Arr: " << some_random_arr_ << "\n";
	memcpy(EmuMath::Helpers::vector_data(some_vector_) + 2, some_random_arr_.data(), sizeof(float) * 3);
	std::cout << newvector_a_ << " | " << newvector_b_ << " | " << newvector_c_ << " | " << some_vector_ << "\n";
	...
	//EmuMath::Helpers::vector_copy(some_newvec4f_, 15.0f);
	some_newvec4f_.Copy(15.0f);
	std::cout << some_newvec4f_ << "\n";
	//EmuMath::Helpers::vector_copy(some_newvec4f_, some_newvec3f_);
	some_newvec4f_.Copy(some_newvec3f_);
	some_newvec4f_.at<3>() = 25.0f;
	std::cout << some_newvec4f_ << "\n";
	EmuMath::NewVector<2, float> some_newvec2f_(-77.7f, +77.7f);
	//EmuMath::Helpers::vector_copy<1, 3>(some_newvec4f_, some_newvec2f_);
	some_newvec4f_.Copy<1, 3>(some_newvec2f_);
	std::cout << some_newvec4f_ << "\n";
	//EmuMath::Helpers::vector_copy<false>(some_newvec4f_, some_newvec3f_);
	some_newvec4f_.Copy<false>(some_newvec3f_);
	std::cout << some_newvec4f_ << "\n";
	//EmuMath::Helpers::vector_copy<true>(some_newvec4f_, some_newvec2f_);
	some_newvec4f_.Copy<true>(some_newvec2f_);
	std::cout << some_newvec4f_ << "\n";
	some_newvec4f_ = decltype(some_newvec4f_)(5, 10, 15, 20);
	std::cout << some_newvec4f_ << "\n";
	//EmuMath::Helpers::vector_copy_from_start<1>(some_newvec4f_, some_newvec2f_);
	some_newvec4f_.CopyFromStart<1>(some_newvec2f_);
	std::cout << some_newvec4f_ << "\n";
	some_newvec4f_ = decltype(some_newvec4f_)(5, 10, 15, 20);
	std::cout << some_newvec4f_ << "\n";
	//EmuMath::Helpers::vector_copy_from_start_contained_only<1>(some_newvec4f_, some_newvec2f_);
	...
	//constexpr auto shuffled_newvec4f_ = EmuMath::Helpers::vector_shuffle<3, 2, 1, 0>(another_newvec4f_);
	constexpr auto shuffled_newvec4f_ = another_newvec4f_.Shuffle<3, 2, 1, 0>();
	//constexpr auto shuffled_newvec8d_ = EmuMath::Helpers::vector_shuffle<double, 0, 2, 1, 3, 1, 1, 0, 0>(another_newvec4f_);
	...
	//auto shuffled_newvec7f_ref_ = EmuMath::Helpers::vector_shuffle_theoretical<float&, 0, 1, 2, 3, 2, 1, 0>(runtime_other_newvec4f_);
	...
	std::cout << runtime_other_newvec4f_ << "\n";
	std::cout << shuffled_newvec7f_ref_ << "\n";
	//EmuMath::NewVector<12, long double> some_newvec12ld_(EmuMath::Helpers::vector_shuffle_theoretical<float, 0, 1, 2, 3, 4, 5, 6, 7>(shuffled_newvec7f_ref_));
	EmuMath::NewVector<12, long double> some_newvec12ld_(shuffled_newvec7f_ref_.ShuffleTheoretical<float, 0, 1, 2, 3, 4, 5, 6, 7>());
	//std::cout << EmuMath::Helpers::vector_shuffle_theoretical<0, 1, 2, 3, 4, 5, 6, 7>(shuffled_newvec7f_ref_) << "\n";
	...
	constexpr auto new_vec_floored_ = EmuMath::Helpers::vector_floor_constexpr(new_vec_to_round_);
	constexpr auto new_vec_ceiled_ = EmuMath::Helpers::vector_ceil_constexpr(new_vec_to_round_);
	constexpr auto new_vec_trunced_ = EmuMath::Helpers::vector_trunc_constexpr(new_vec_to_round_);
	constexpr auto new_vec_abs_ = EmuMath::Helpers::vector_abs(new_vec_to_round_);



	std::cout << "---\n";
	constexpr auto pre_negate_vec_ = EmuMath::make_vector<float>(-5.0f, 0.0f, 12.0f, -66.66f);
	constexpr auto full_negate_vec_ = EmuMath::Helpers::vector_negate(pre_negate_vec_);
	constexpr auto partial_negate_vec_ = EmuMath::Helpers::vector_negate_range<27, long double, 3, 4>(pre_negate_vec_);

	constexpr auto some_other_negation_ = EmuMath::Helpers::vector_mutate<4, long double>(EmuCore::do_negate<void>(), EmuMath::make_vector<float>(1, -2, 3, -4));
	constexpr auto negated_negation_ = some_other_negation_.Negate();
	constexpr auto negated_negation_full_range_ = EmuMath::Helpers::vector_negate_range<0, 10>(some_other_negation_);
	constexpr auto negated_negation_full_range_mut_ = EmuMath::Helpers::vector_mutate_range<4, float, 0, 10>(EmuCore::do_negate<void>(), some_other_negation_);
	constexpr auto negated_negated_negation_full_range_mut_ = EmuMath::Helpers::vector_mutate_range<4, float, 0, 10, 1>(EmuCore::do_negate<void>(), negated_negation_full_range_mut_);
	constexpr auto another_negation_again_ = EmuMath::Helpers::vector_negate_range_no_copy<4, float, 2, 4, 0>(some_other_negation_);
	...
	constexpr auto a_mess_trunced_ = EmuMath::Helpers::vector_trunc_range_no_copy_constexpr<2, 4, 1>(a_mess_);
	constexpr auto a_mess_ceiled_ = EmuMath::Helpers::vector_ceil_range_no_copy_constexpr<2, 4, 1>(a_mess_);
	constexpr auto a_mess_floored_ = EmuMath::Helpers::vector_floor_range_no_copy_constexpr<2, 4, 1>(a_mess_);
	constexpr auto a_mess_absed_ = EmuMath::Helpers::vector_abs_range_no_copy<2, 4, 1>(a_mess_);
	...
	//EmuMath::Helpers::vector_lerp(another_new_vec_4d_, another_new_vec_4d_, EmuMath::NewVector<4, float>(10, 20, 30, 40), 0.5);
	another_new_vec_4d_.Lerp(another_new_vec_4d_, EmuMath::NewVector<4, float>(10, 20, 30, 40), 0.5);
	std::cout << another_new_vec_4d_ << "\n";

	std::cout << "\n\n";
	//constexpr auto new_add_res_a_ = EmuMath::Helpers::vector_add(EmuMath::make_vector<float>(1.5f, 0.0, 3.9f, -2.5f, -5.0f, 2.5f), 2.5L);
	constexpr auto new_add_res_a_ = EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5) + 2.5;
	//constexpr auto new_add_res_b_ = EmuMath::Helpers::vector_add_range<1, 5>(EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5), 2.5);
	constexpr auto new_add_res_b_ = EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5).AddRange<1, 5>(2.5);
	//constexpr auto new_add_res_c_ = EmuMath::Helpers::vector_add_range_no_copy<1, 5, 0>(EmuMath::make_vector<float>(1.5, 0, 3.9, -2.5, -5, 2.5), 2.5);
	constexpr auto new_add_res_c_ = EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5).AddRangeNoCopy<1, 5, 0>(2.5);

	//constexpr auto new_sub_res_a_ = EmuMath::Helpers::vector_subtract(EmuMath::make_vector<float>(1.5f, 0.0, 3.9f, -2.5f, -5.0f, 2.5f), 2.5L);
	constexpr auto new_sub_res_a_ = EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5) - 2.5L;
	//constexpr auto new_sub_res_b_ = EmuMath::Helpers::vector_subtract_range<1, 5>(EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5), 2.5);
	constexpr auto new_sub_res_b_ = EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5).SubtractRange<1, 5>(2.5);
	//constexpr auto new_sub_res_c_ = EmuMath::Helpers::vector_subtract_range_no_copy<1, 5, 0>(EmuMath::make_vector<float>(1.5, 0, 3.9, -2.5, -5, 2.5), 2.5);
	constexpr auto new_sub_res_c_ = EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5).SubtractRangeNoCopy<1, 5, 0>(2.5);

	//constexpr auto new_mul_res_a_ = EmuMath::Helpers::vector_multiply(EmuMath::make_vector<float>(1.5f, 0.0, 3.9f, -2.5f, -5.0f, 2.5f), 2.5L);
	constexpr auto new_mul_res_a_ = EmuMath::make_vector<float>(1.5f, 0.0, 3.9f, -2.5f, -5.0f, 2.5f) * 2.5L;
	//constexpr auto new_mul_res_b_ = EmuMath::Helpers::vector_multiply_range<1, 5>(EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5), 2.5);
	constexpr auto new_mul_res_b_ = EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5).MultiplyRange<1, 5>(2.5);
	//constexpr auto new_mul_res_c_ = EmuMath::Helpers::vector_multiply_range_no_copy<1, 5, 0>(EmuMath::make_vector<float>(1.5, 0, 3.9, -2.5, -5, 2.5), 2.5);
	constexpr auto new_mul_res_c_ = EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5).MultiplyRangeNoCopy<1, 5, 0>(2.5);

	//constexpr auto new_div_res_a_ = EmuMath::Helpers::vector_divide(EmuMath::make_vector<float>(1.5f, 0.0, 3.9f, -2.5f, -5.0f, 2.5f), 2.5L);
	constexpr auto new_div_res_a_ = EmuMath::make_vector<float>(1.5f, 0.0, 3.9f, -2.5f, -5.0f, 2.5f) / 2.5L;
	//constexpr auto new_div_res_b_ = EmuMath::Helpers::vector_divide_range<1, 5>(EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5), 2.5);
	constexpr auto new_div_res_b_ = EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5).DivideRange<1, 5>(2.5);
	//constexpr auto new_div_res_c_ = EmuMath::Helpers::vector_divide_range_no_copy<1, 5, 0>(EmuMath::make_vector<float>(1.5, 0, 3.9, -2.5, -5, 2.5), 2.5);
	constexpr auto new_div_res_c_ = EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5).DivideRangeNoCopy<1, 5, 0>(2.5);

	//constexpr auto new_mod_res_a_ = EmuMath::Helpers::vector_mod(EmuMath::make_vector<float>(1.5f, 0.0, 3.9f, -2.5f, -5.0f, 2.5f), 2.5L);
	constexpr auto new_mod_res_a_ = EmuMath::make_vector<float>(1.5f, 0.0, 3.9f, -2.5f, -5.0f, 2.5f) % 2.5L;
	//constexpr auto new_mod_res_b_ = EmuMath::Helpers::vector_mod_range<1, 5>(EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5), 2.5);
	constexpr auto new_mod_res_b_ = EmuMath::make_vector<float>(1.5, 0.0, 3.9, -2.5, -5, 2.5).ModRange<1, 5>(2.5);
	//constexpr auto new_mod_res_c_ = EmuMath::Helpers::vector_mod_range_no_copy<1, 5, 0>(EmuMath::make_vector<float>(1.5, 0, 3.9, -2.5, -5, 2.5), 2.5);
	...
	//constexpr auto new_and_res_a_ = EmuMath::Helpers::vector_bitwise_and(new_bitwise_test_, 3);
	constexpr auto new_and_res_a_ = new_bitwise_test_ & 3;

	//constexpr auto new_or_res_a_ = EmuMath::Helpers::vector_bitwise_or(new_bitwise_test_, 3);
	constexpr auto new_or_res_a_ = new_bitwise_test_ | 3;

	//constexpr auto new_xor_res_a_ = EmuMath::Helpers::vector_bitwise_xor(new_bitwise_test_, 3);
	constexpr auto new_xor_res_a_ = new_bitwise_test_ ^ 3;

	//constexpr auto new_lshift_res_a_ = EmuMath::Helpers::vector_shift_left(new_bitwise_test_, 2);
	constexpr auto new_lshift_res_a_ = new_bitwise_test_ << 2;

	//constexpr auto new_rshift_res_a_ = EmuMath::Helpers::vector_shift_right(new_bitwise_test_, 2);
	constexpr auto new_rshift_res_a_ = new_bitwise_test_ >> 2;

	//constexpr auto new_not_res_a_ = EmuMath::Helpers::vector_bitwise_not(EmuMath::make_vector<std::uint16_t>(0xFFFF, 0xFFFE, 0));
	...
	constexpr auto vec_up_left_ = EmuMath::Helpers::vector_lerp(vec_left_, vec_up_, 0.5f);
	constexpr auto vec_angle_cos_a_ = EmuMath::Helpers::vector_angle_cos_constexpr(vector_for_mag_, vector_for_mag_);
	constexpr auto vec_angle_cos_b_ = EmuMath::Helpers::vector_angle_cos_constexpr(vec_left_, vec_up_);
	constexpr auto vec_angle_cos_c_ = EmuMath::Helpers::vector_angle_cos_constexpr(vec_up_left_, vec_up_);
	...
	//constexpr auto min_a_ = EmuMath::Helpers::vector_min(vec_for_min_a_, -1);
	constexpr auto min_a_ = vec_for_min_a_.Min(-1);
	//constexpr auto min_b_ = EmuMath::Helpers::vector_min(vec_for_min_b_, -1.5L);
	constexpr auto min_b_ = vec_for_min_b_.Min(-1.5L);
	//constexpr auto min_c_ = EmuMath::Helpers::vector_min(vec_for_min_a_, vec_for_min_b_);
	constexpr auto min_c_ = vec_for_min_a_.Min(vec_for_min_b_);

	//constexpr auto min_range_a_ = EmuMath::Helpers::vector_min_range<1, 3>(vec_for_min_a_, vec_for_min_b_);
	constexpr auto min_range_a_ = vec_for_min_a_.MinRange<1, 3>(vec_for_min_b_);
	//constexpr auto min_range_b_ = EmuMath::Helpers::vector_min_range<1, 3>(vec_for_min_b_, vec_for_min_a_);
	constexpr auto min_range_b_ = vec_for_min_b_.MinRange<1, 3>(vec_for_min_a_);

	//constexpr auto min_range_no_copy_a_ = EmuMath::Helpers::vector_min_range_no_copy<1, 3, 0>(vec_for_min_a_, vec_for_min_b_);
	constexpr auto min_range_no_copy_a_ = vec_for_min_a_.MinRangeNoCopy<1, 3, 0>(vec_for_min_b_);
	//constexpr auto min_range_no_copy_b_ = EmuMath::Helpers::vector_min_range_no_copy<1, 3, 0>(vec_for_min_b_, vec_for_min_a_);
	constexpr auto min_range_no_copy_b_ = vec_for_min_b_.MinRangeNoCopy<1, 3, 0>(vec_for_min_a_);


	constexpr auto max_a_ = EmuMath::Helpers::vector_max(vec_for_min_a_, -1);
	constexpr auto max_b_ = EmuMath::Helpers::vector_max(vec_for_min_b_, -1.5L);
	constexpr auto max_c_ = EmuMath::Helpers::vector_max(vec_for_min_a_, vec_for_min_b_);

	constexpr auto max_range_a_ = EmuMath::Helpers::vector_max_range<1, 3>(vec_for_min_a_, vec_for_min_b_);
	constexpr auto max_range_b_ = EmuMath::Helpers::vector_max_range<1, 3>(vec_for_min_b_, vec_for_min_a_);

	constexpr auto max_range_no_copy_a_ = EmuMath::Helpers::vector_max_range_no_copy<1, 3, 0>(vec_for_min_a_, vec_for_min_b_);
	constexpr auto max_range_no_copy_b_ = EmuMath::Helpers::vector_max_range_no_copy<1, 3, 0>(vec_for_min_b_, vec_for_min_a_);
	...
	try
	{
		std::cout << concat_result_ab_.at(std::numeric_limits<std::size_t>::max()) << "\n";
	}
	...
	constexpr auto vec_cast_4f_ = EmuMath::Helpers::vector_cast<4, float>(vec_to_cast_);
	constexpr auto vec_cast_20f_ = EmuMath::Helpers::vector_cast<20, float>(vec_to_cast_);
	constexpr auto vec_cast_xld_ = EmuMath::Helpers::vector_cast<long double>(vec_to_cast_);
	constexpr auto vec_cast_xf_offset_ = EmuMath::Helpers::vector_cast<float, 5>(vec_to_cast_);
	constexpr auto rval_vec_cast_4f_offset_ = EmuMath::Helpers::vector_cast<5, float, 3>(EmuMath::make_vector<std::uint16_t>(1, 2, 3, 4, 5, 6, 7, 8));
	...
	//constexpr bool cmp_result_0_ = EmuMath::Helpers::vector_cmp_all<EmuCore::do_cmp_equal_to>(cmp_vec_a_, cmp_vec_b_);
	...
	//constexpr auto norm_p0p1p2_no_norm_ = EmuMath::Helpers::vector_normal_to_plane_3d_no_norm(p0_, p1_, p2_);
	constexpr auto norm_p0p1p2_no_norm_ = p0_.NormalToPlane3NoNorm(p1_, p2_);
	//constexpr auto norm_p0p1p2_ = EmuMath::Helpers::vector_normal_to_plane_3d_constexpr(p0_, p1_, p2_);
	...
	constexpr auto vector_a_ = EmuMath::NewVector<2, double>(0.5, 0.25);
	...
	constexpr auto some_vec_ = EmuMath::NewVector<1, long double>(colour_a_.AsVector());
