#ifndef EMU_MATH_SIMD_HELPERS_TEMPLATE_CAST_H_INC_
#define EMU_MATH_SIMD_HELPERS_TEMPLATE_CAST_H_INC_ 1
...
namespace EmuMath::SIMD
{
	...
	[[nodiscard]] inline ToRegister_ cast(FromRegister_ from_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<from_uq>)
		{
			...
			{
				static_assert(false, "Attempted to perform EmuMath::SIMD::cast, but the provided ToRegister_ (the output type) is not recognised as a supported SIMD register.");
			}
		}
		else
		{
			static_assert(false, "Attempted to perform EmuMath::SIMD::cast, but the provided FromRegister_ (the input type) is not recognised as a supported SIMD register.");
		}
	}
}
