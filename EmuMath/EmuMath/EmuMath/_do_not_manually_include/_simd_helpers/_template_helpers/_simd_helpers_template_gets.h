#ifndef EMU_MATH_SIMD_HELPERS_TEMPLATE_GETS_H_INC_
#define EMU_MATH_SIMD_HELPERS_TEMPLATE_GETS_H_INC_ 1
...
namespace EmuMath::SIMD
{
	...
	[[nodiscard]] inline Out_ get_index(Register_ register_)
	{
		...
		{
			static_assert(false, "Attempted to extract a SIMD register index via EmuMath::SIMD::get_index, but the provided register_ is not recognised as a SIMD register.");
		}
	}
	...
	/// </para>
	/// <para> EmuMath::SIMD::TMP::simd_register_width_v may be useful for forming a generic output location for this function. </para>
	/// </summary>
	...
	/// <param name="p_out_non_const_">Non-const pointer to a memory location to output to.</param>
