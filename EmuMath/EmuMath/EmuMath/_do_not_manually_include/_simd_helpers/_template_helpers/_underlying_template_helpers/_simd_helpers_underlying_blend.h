#ifndef EMU_MATH_SIMD_HELPERS_UNDERLYING_BLEND_H_INC_
#define EMU_MATH_SIMD_HELPERS_UNDERLYING_BLEND_H_INC_ 1
			...
			if constexpr (_underlying_simd_helpers::_is_valid_blend_mask<mask_generator>::value)
			{
				if constexpr (_underlying_simd_helpers::_blend_mask_is_simd_register<mask_generator>::value)
				{
					...
					else if constexpr (EmuCore::TMPHelpers::is_any_comparison_true<std::is_same, register_type_uq, __m128i, __m256i, __m512i>::value)
					{
						...
						if constexpr (EmuMath::SIMD::TMP::_valid_simd_int_element_width(per_element_width_))
						{
				...
				else
				{
					...
					else if constexpr (EmuCore::TMPHelpers::is_any_comparison_true<std::is_same, register_type_uq, __m128i, __m256i, __m512i>::value)
					{
						...
						if constexpr (EmuMath::SIMD::TMP::_assert_valid_simd_int_element_width<per_element_width_>() && (per_element_width_ != 8))
						{
					...
					{
						static_assert(false, "Attempted to execute a SIMD blend via EmuMath SIMD helpers using a SIMD register that is not supported for this operation.");
					}
				}
			}
			else
