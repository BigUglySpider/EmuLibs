#ifndef EMU_MATH_COMMON_UNDERLYING_SIMD_CAST_INCLUDES_H_INC_
#define EMU_MATH_COMMON_UNDERLYING_SIMD_CAST_INCLUDES_H_INC_ 1
...
namespace EmuMath::SIMD::_underlying_simd_helpers
{
	...
	[[nodiscard]] inline ToRegister_ _cast_m128(__m128 from_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<to_uq>)
		{
			...
			{
				static_assert(false, "Attempted to cast a __m128 register via EmuMath SIMD helpers, but the provided SIMD register to cast to is not supported for this operation.");
			}
		}
		else
		{
			static_assert(false, "Attempted to cast a __m128 register via EmuMath SIMD helpers, but the provided register to cast to is not recognisd as a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] inline ToRegister_ _cast_m256(__m256 from_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<to_uq>)
		{
			...
			{
				static_assert(false, "Attempted to cast a __m256 register via EmuMath SIMD helpers, but the provided SIMD register to cast to is not supported for this operation.");
			}
		}
		else
		{
			static_assert(false, "Attempted to cast a __m256 register via EmuMath SIMD helpers, but the provided register to cast to is not recognisd as a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] inline ToRegister_ _cast_m512(__m512 from_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<to_uq>)
		{
			...
			{
				static_assert(false, "Attempted to cast a __m512 register via EmuMath SIMD helpers, but the provided SIMD register to cast to is not supported for this operation.");
			}
		}
		else
		{
			static_assert(false, "Attempted to cast a __m512 register via EmuMath SIMD helpers, but the provided register to cast to is not recognisd as a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] inline ToRegister_ _cast_m128d(__m128d from_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<to_uq>)
		{
			...
			{
				static_assert(false, "Attempted to cast a __m128d register via EmuMath SIMD helpers, but the provided SIMD register to cast to is not supported for this operation.");
			}
		}
		else
		{
			static_assert(false, "Attempted to cast a __m128d register via EmuMath SIMD helpers, but the provided register to cast to is not recognisd as a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] inline ToRegister_ _cast_m256d(__m256d from_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<to_uq>)
		{
			...
			{
				static_assert(false, "Attempted to cast a __m256d register via EmuMath SIMD helpers, but the provided SIMD register to cast to is not supported for this operation.");
			}
		}
		else
		{
			static_assert(false, "Attempted to cast a __m256d register via EmuMath SIMD helpers, but the provided register to cast to is not recognisd as a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] inline ToRegister_ _cast_m512d(__m512d from_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<to_uq>)
		{
			...
			{
				static_assert(false, "Attempted to cast a __m512d register via EmuMath SIMD helpers, but the provided SIMD register to cast to is not supported for this operation.");
			}
		}
		else
		{
			static_assert(false, "Attempted to cast a __m512d register via EmuMath SIMD helpers, but the provided register to cast to is not recognisd as a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] inline ToRegister_ _cast_m128i(__m128i from_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<to_uq>)
		{
			...
			{
				static_assert(false, "Attempted to cast a __m128i register via EmuMath SIMD helpers, but the provided SIMD register to cast to is not supported for this operation.");
			}
		}
		else
		{
			static_assert(false, "Attempted to cast a __m128i register via EmuMath SIMD helpers, but the provided register to cast to is not recognisd as a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] inline ToRegister_ _cast_m256i(__m256i from_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<to_uq>)
		{
			...
			{
				static_assert(false, "Attempted to cast a __m256i register via EmuMath SIMD helpers, but the provided SIMD register to cast to is not supported for this operation.");
			}
		}
		else
		{
			static_assert(false, "Attempted to cast a __m256i register via EmuMath SIMD helpers, but the provided register to cast to is not recognisd as a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] inline ToRegister_ _cast_m512i(__m512i from_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<to_uq>)
		{
			...
			{
				static_assert(false, "Attempted to cast a __m512i register via EmuMath SIMD helpers, but the provided SIMD register to cast to is not supported for this operation.");
			}
		}
		else
		{
			static_assert(false, "Attempted to cast a __m512i register via EmuMath SIMD helpers, but the provided register to cast to is not recognisd as a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] ToRegister_ _cast(FromRegister_ from_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<from_uq>)
		{
			using to_uq = EmuCore::TMPHelpers::remove_ref_cv_t<ToRegister_>;
			if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<to_uq>)
			{
				...
				{
					static_assert(false, "Attempted to cast a SIMD register via EmuMath SIMD helpers, but the provided SIMD register to cast from is not supported for this operation.");
				}
			}
			else
			{
				static_assert(false, "Attempted to cast a SIMD register via EmuMath SIMD helpers, but the provided register to cast to is not recognisd as a supported SIMD register.");
			}
		}
		else
		{
			static_assert(false, "Attempted to cast a SIMD register via EmuMath SIMD helpers, but the provided register to cast from is not recognisd as a supported SIMD register.");
		}
	}
}
