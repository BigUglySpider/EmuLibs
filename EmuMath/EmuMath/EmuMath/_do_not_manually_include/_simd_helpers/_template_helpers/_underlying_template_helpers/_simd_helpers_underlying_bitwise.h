#ifndef EMU_MATH_SIMD_HELPERS_UNDERLYING_BITWISE_H_INC_
#define EMU_MATH_SIMD_HELPERS_UNDERLYING_BITWISE_H_INC_ 1
...
namespace EmuMath::SIMD::_underlying_simd_helpers
{
	...
	[[nodiscard]] inline Register_ _and(Register_ lhs_, Register_ rhs_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<register_type_uq>)
		{
			...
			{
				static_assert(false, "Attempted to perform bitwise AND of a SIMD register using EmuMath SIMD helpers, but the provided SIMD register is not supported for this operation");
			}
		}
		else
		{
			static_assert(false, "Attempted to perform bitwise AND of a SIMD register using EmuMath SIMD helpers, but provided a type that is not a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] inline Register_ _or(Register_ lhs_, Register_ rhs_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<register_type_uq>)
		{
			...
			{
				static_assert(false, "Attempted to perform bitwise OR of a SIMD register using EmuMath SIMD helpers, but the provided SIMD register is not supported for this operation");
			}
		}
		else
		{
			static_assert(false, "Attempted to perform bitwise OR of a SIMD register using EmuMath SIMD helpers, but provided a type that is not a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] inline Register_ _xor(Register_ lhs_, Register_ rhs_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<register_type_uq>)
		{
			...
			{
				static_assert(false, "Attempted to perform bitwise XOR of a SIMD register using EmuMath SIMD helpers, but the provided SIMD register is not supported for this operation");
			}
		}
		else
		{
			static_assert(false, "Attempted to perform bitwise XOR of a SIMD register using EmuMath SIMD helpers, but provided a type that is not a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] inline Register_ _andnot(Register_ lhs_, Register_ rhs_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<register_type_uq>)
		{
			...
			{
				static_assert(false, "Attempted to perform bitwise ANDNOT of a SIMD register using EmuMath SIMD helpers, but the provided SIMD register is not supported for this operation");
			}
		}
		else
		{
			static_assert(false, "Attempted to perform bitwise ANDNOT of a SIMD register using EmuMath SIMD helpers, but provided a type that is not a supported SIMD register.");
		}
	}
	...
	[[nodiscard]] inline Register_ _not(Register_ to_not_)
	{
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<register_type_uq>)
		{
			...
			if constexpr (is_supported_register_)
			{
				...
				{
					static_assert(false, "Attempted to perform bitwise NOT of a SIMD register using EmuMath SIMD helpers, but a mask to perform the operation could not be successfully generated.");
				}
			}
			else
			{
				static_assert(false, "Attempted to perform bitwise NOT of a SIMD register using EmuMath SIMD helpers, but provided a SIMD register that is not supported for this operation.");
			}
		}
		else
		{
			static_assert(false, "Attempted to perform bitwise NOT of a SIMD register using EmuMath SIMD helpers, but provided a type that is not a supported SIMD register.");
		}
	}
}
