#ifndef EMU_MATH_SIMD_TEMPLATE_HELPERS_UNDERLYING_SHUFFLE_H_INC_
#define EMU_MATH_SIMD_TEMPLATE_HELPERS_UNDERLYING_SHUFFLE_H_INC_ 1
...
namespace EmuMath::SIMD::_underlying_simd_helpers
{
	...
	[[nodiscard]] inline Register_ _execute_shuffle(Register_ a_, Register_ b_)
	{
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<Register_>)
		{
			...
			using shuffle_mask_inst = EmuMath::SIMD::_underlying_simd_helpers::_shuffle_mask<register_type_uq, Indices_...>;
			if constexpr (EmuMath::SIMD::_underlying_simd_helpers::is_valid_shuffle_mask_instance<shuffle_mask_inst>::value)
			{
				...
				{
					static_assert(false, "Attempted to shuffle SIMD registers using EmuMath SIMD helpers, but the provided Register_ types are unsupported.");
				}
			}
			else
			{
				static_assert(false, "Attempted to shuffle SIMD registers using EmuMath SIMD helpers, but the provided indices did not form a valid shuffle mask template for the provided register type.");
			}
		}
		else
		{
			static_assert(false, "Attempted to shuffle SIMD registers using EmuMath SIMD helpers, but the provided Register_ type was not a supported SIMD register type.");
		}
	}
	template<std::size_t...Indices_, class Register_>
		...
		if constexpr (EmuMath::SIMD::TMP::is_simd_register_v<Register_>)
		{
			...
			else
			{
				...
				if constexpr (EmuMath::SIMD::_underlying_simd_helpers::is_valid_shuffle_mask_instance<shuffle_mask_inst>::value)
				{
					...
					{
						static_assert(false, "Attempted to shuffle a SIMD register using EmuMath SIMD helpers, but the provided Register_ type is unsupported.");
					}
				}
				else
				{
					static_assert(false, "Attempted to shuffle a SIMD register using EmuMath SIMD helpers, but the provided indices did not form a valid shuffle mask template for the provided register type.");
				}
			}
		}
		else
		{
			static_assert(false, "Attempted to shuffle a SIMD register using EmuMath SIMD helpers, but the provided Register_ type was not a supported SIMD register type.");
		}
	}
