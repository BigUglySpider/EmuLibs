...
#pragma region ALIASES_AND_STATIC_CONSTANT_EXPRESSIONS
	...
	public:
		...
		using emu_vector_type = Vector3<T>;
		...
		using copy_vector = Vector3<nonref_value_type_without_qualifiers>;
#pragma endregion
...
#pragma region CONSTRUCTORS
		...
		/// <param name="toCopy">Vector to copy the elements of.</param>
		template<typename OtherT>
		explicit constexpr Vector3(const Vector4<OtherT>& toCopy) :
			Vector3(toCopy.x, toCopy.y, toCopy.z)
		{
		}
		/// <summary> Constructs a 3-dimensional vector with its x, y and z components set to copies of the passed vector's respective components, performing casts where needed. </summary>
		...
		/// <param name="toCopy">Vector to copy the elements of.</param>
		template<typename OtherT>
		explicit constexpr Vector3(Vector4<OtherT>& toCopy) :
			Vector3(toCopy.x, toCopy.y, toCopy.z)
		{
		}
		/// <summary>
...
#pragma region OVERALL_OPERATIONS
		...
		constexpr OutT Mean() const
		{
			...
			{
				return (x + y + z) * size_reciprocal<OutT>();
			}
			else
			{
				return static_cast<OutT>((x + y + z) * size_reciprocal());
			}
		}
...
#pragma region VECTOR_OPERATIONS
		...
		Vector3<OutFP> Normalise() const
		{
			return EmuMath::Helpers::VectorNormalise<OutFP>(*this);
		}
		template<typename OutFP = default_floating_point>
		constexpr Vector3<OutFP> NormaliseConstexpr() const
		{
