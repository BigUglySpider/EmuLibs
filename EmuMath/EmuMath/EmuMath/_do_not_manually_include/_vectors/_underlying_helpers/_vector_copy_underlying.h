...
#pragma region ASSIGNMENT_COPY
	...
	template<std::size_t CopyBegin_, std::size_t CopyEnd_, std::size_t ReadOffset_, class OutVector_, class In_, std::size_t...Indices_>
	constexpr inline void _vector_copy_assign_execution(std::index_sequence<Indices_...> indices_, OutVector_& out_, In_&& in_)
	{
		(_vector_copy_index_in_range<Indices_, CopyBegin_, CopyEnd_, ReadOffset_, true>(out_, std::forward<In_>(in_)), ...);
	}
	...
	{
		_vector_copy_assign_execution<CopyBegin_, CopyEnd_, ReadOffset_>(std::make_index_sequence<OutSize_>(), out_, std::forward<In_>(in_));
	}
#pragma endregion
}
