...
namespace EmuMath::Helpers::_vector_underlying
{
	...
	template<std::size_t Index_, class VectorArg_, std::size_t Size_, typename T_>
	constexpr inline void _vector_set_vector_move(EmuMath::NewVector<Size_, T_>& out_vector_, VectorArg_&& in_vector_arg_)
	{
		using lhs_vector_type = EmuMath::NewVector<Size_, T_>;
		if constexpr (Index_ < lhs_vector_type::size)
		{
			...
			if constexpr (Index_ < rhs_vector_type::size)
			{
				...
				_vector_set_vector_move<Index_ + 1>(out_vector_, std::move(in_vector_arg_));
			}
			else
			{
				auto empty_value_ = _vector_get_non_contained_value<rhs_vector_type>();
				_vector_set_all_execution<Index_, lhs_vector_type::size>(out_vector_, empty_value_);
			}
		}
	}
	...
	constexpr inline void _vector_set_vector_copy(EmuMath::NewVector<Size_, T_>& out_vector_, VectorArg_& in_vector_arg_)
	{
		...
		if constexpr (Index_ < lhs_vector_type::size)
		{
			...
			if constexpr (Index_ < rhs_vector_type::size)
			{
				...
				_vector_set_vector_copy<Index_ + 1>(out_vector_, in_vector_arg_);
			}
			else
			{
				auto empty_value_ = _vector_get_non_contained_value<rhs_vector_type>();
	...
	template<class VectorArg_, std::size_t Size_, typename T_>
			...
			if constexpr (lhs_vector_type::contains_ref)
			{
				...
				else
				{
					if constexpr (!lhs_vector_type::contains_const_ref)
					{
						...
						{
							_vector_set_vector_copy<0, VectorArg_>(out_vector_, vector_arg_);
						}
					}
					else
					{
					...
					else
					{
						if constexpr (!lhs_vector_type::contains_const_ref)
						{
							...
							else
							{
								...
								{
									_vector_set_vector_move<0>(out_vector_, std::forward<vector_arg_no_ref>(vector_arg_));
								}
								else
								{
									_vector_set_vector_copy<0>(out_vector_, vector_arg_);
								}
							}
						}
						else
