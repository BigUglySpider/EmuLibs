...
#pragma region VALIDITY_CHECKS
	...
	[[nodiscard]] constexpr inline bool new_vector_cast_is_valid()
	{
	...
	[[nodiscard]] constexpr inline bool new_vector_cast_is_valid()
	{
		if constexpr (EmuMath::TMP::is_emu_new_vector_v<OutVector_>)
		{
			using out_args = EmuMath::TMP::emu_vector_template_args<OutVector_>;
			return new_vector_cast_is_valid<InSize_, InT_, out_args::Size_, typename out_args::T_, InIsConst_, InIsLvalRef_, Offset_>();
		}
		else
		{
			return false;
	...
	[[nodiscard]] constexpr inline bool new_vector_cast_is_valid()
	{
		if constexpr (EmuCore::TMP::are_all_check<EmuMath::TMP::is_emu_new_vector, InVector_, OutVector_>::value)
		{
			using out_args = EmuMath::TMP::emu_vector_template_args<OutVector_>;
...
#pragma region IS_STATIC_CASTABLE_SPECIALISATIONS
	...
	{
		static constexpr bool value = EmuMath::Helpers::new_vector_cast_is_valid
		<
