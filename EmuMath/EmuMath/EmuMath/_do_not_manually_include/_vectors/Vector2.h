...
#pragma region CONSTRUCTORS
		...
		constexpr Vector(this_type& toCopy_) : x(toCopy_.x), y(toCopy_.y)
		{
		}
		template<typename = std::enable_if_t<!contains_non_const_reference_wrappers>>
		constexpr Vector(const this_type& toCopy_) : x(toCopy_.x), y(toCopy_.y)
		{
		}
			...
			y(static_cast<contained_type>(std::forward<Y_>(y_)))
		{
			static_assert
			(
				EmuCore::TMP::are_all_comparisons_true<std::is_constructible, contained_type, X_, Y_>::value,
...
#pragma endregion
	...
	private:
		template<std::size_t Index_>
		[[nodiscard]] constexpr inline contained_type& _get_index()
		{
		...
		template<std::size_t Index_>
		[[nodiscard]] constexpr inline const contained_type& _get_index() const
		{
			if constexpr (Index_ == 0)
			{
				return x;
		...
		[[nodiscard]] constexpr inline contained_type& _get_index(const std::size_t index_)
		{
			return *(data() + index_);
		}
		[[nodiscard]] constexpr inline const contained_type& _get_index(const std::size_t index_) const
		{
			return *(data() + index_);
		}
