...
namespace EmuMath::Helpers
{
	...
	[[nodiscard]] constexpr inline EmuMath::Matrix<OutNumColumns_, OutNumRows_, out_contained_type, OutColumnMajor_> matrix_copy(const Src_& src_)
	{
	...
	constexpr inline LhsMatrix_& matrix_copy(LhsMatrix_& lhs_, const Rhs_& rhs_)
	{
	...
	template<std::size_t OutNumColumns_, std::size_t OutNumRows_, typename out_contained_type, bool OutColumnMajor_ = true, class Src_>
	constexpr inline EmuMath::Matrix<OutNumColumns_, OutNumRows_, out_contained_type, OutColumnMajor_> matrix_set(Src_& src_)
	{
	...
	template<class LhsMatrix_, class Rhs_>
	constexpr inline LhsMatrix_& matrix_set(LhsMatrix_& lhs_, Rhs_& rhs_)
	{
		if constexpr (EmuMath::TMP::is_emu_matrix_v<LhsMatrix_>)
		{
	...
	template<class LhsMatrix_, class Rhs_>
	constexpr inline LhsMatrix_& matrix_set(LhsMatrix_& lhs_, const Rhs_& rhs_)
	{
		if constexpr (EmuMath::TMP::is_emu_matrix_v<LhsMatrix_>)
		{
	...
	[[nodiscard]] constexpr inline EmuMath::ConstRefMatrix<NumColumns_, NumRows_, T_, ColumnMajor_> matrix_make_const_ref(Args_&&...contiguous_args_)
	{
		...
		{
			static_assert(false, "Attempted to make an EmuMath matrix of const references via matrix_make with a number of arguments not equal to the number of elements within the matrix.");
		}
	}
	...
	template<std::size_t NumColumns_, std::size_t NumRows_, typename T_, bool ColumnMajor_ = true, typename...Args_>
	[[nodiscard]] constexpr inline EmuMath::RefMatrix<NumColumns_, NumRows_, T_, ColumnMajor_> matrix_make_ref(Args_&&...contiguous_args_)
	{
		if constexpr (std::is_const_v<T_>)
		{
			return matrix_make_const_ref<NumColumns_, NumRows_, T_, ColumnMajor_>(contiguous_args_...);
		}
		else
		{
			...
			{
				static_assert(false, "Attempted to make an EmuMath matrix of references via matrix_make with a number of arguments not equal to the number of elements within the matrix.");
			}
		}
	}
	...
	template<std::size_t NumColumns_, std::size_t NumRows_, typename T_, bool ColumnMajor_ = true, typename...Args_>
	[[nodiscard]] constexpr inline EmuMath::Matrix<NumColumns_, NumRows_, T_, ColumnMajor_> matrix_make(Args_&&...contiguous_args_)
	{
		if constexpr (EmuMath::TMP::is_internal_emu_matrix_reference_v<T_>)
		{
			return matrix_make_ref<NumColumns_, NumRows_, typename T_::value_type, ColumnMajor_>(contiguous_args_...);
		}
		else
		{
			...
			{
				static_assert(false, "Attempted to make an EmuMath matrix via matrix_make with a number of arguments not equal to the number of elements within the matrix.");
			}
		}
	}
}
