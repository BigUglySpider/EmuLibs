...
#pragma region VALIDITY_CHECKS
	...
	[[nodiscard]] constexpr inline bool _can_quaternion_add()
	{
		...
		{
			static_assert(!StaticAssert_, "Unable to add two EmuMath Quaternions as the determined calculation functions cannot be invoked with two arguments of the determined calculation type.");
			return false;
		}
		else
		{
			...
			if constexpr (Assigning_)
			{
				...
				if constexpr (!EmuCore::TMP::valid_assign_direct_or_cast<lhs_value_uq, add_result&&, decltype(std::declval<lhs_ref>())>())
				{
			...
			else
			{
				...
				else
				{
					...
					if constexpr (Assigning_)
					{
						...
						using quat_value_uq = typename quat_ref::value_type_uq;
