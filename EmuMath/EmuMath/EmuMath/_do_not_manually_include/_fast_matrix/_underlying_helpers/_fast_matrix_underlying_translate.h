#ifndef EMU_MATH_FAST_MATRIX_UNDERLYING_TRANSLATE_H_INC_
#define EMU_MATH_FAST_MATRIX_UNDERLYING_TRANSLATE_H_INC_ 1
...
namespace EmuMath::Helpers::_fast_matrix_underlying
{
	...
	>
	[[nodiscard]] constexpr inline decltype(auto) _get_element_arg_for_translation_register(Translations_& translations_)
	{
		...
		{
			constexpr std::size_t column_index = _fast_mat_uq::is_column_major ? MajorIndex_ : full_element_index;
			constexpr std::size_t row_index = _fast_mat_uq::is_column_major ? full_element_index : MajorIndex_;
			constexpr std::size_t end_column_index = _fast_mat_uq::num_columns - 1;
			constexpr std::size_t num_translations = std::tuple_size_v<typename EmuCore::TMP::remove_ref_cv<Translations_>::type>;
			if constexpr (column_index == end_column_index && row_index < num_translations)
			{
				using _tuple_get_result = decltype(std::get<row_index>(translations_));
				return std::forward<_tuple_get_result>
				(
					std::get<row_index>(translations_)
				);
			}
			else
			{
	...
	>
	[[nodiscard]] constexpr inline auto _make_translation_register
	(
	...
	) -> typename EmuCore::TMP::remove_ref_cv_t<OutFastMatrix_>::register_type
	{
		...
		(
			_get_element_arg_for_translation_register<MajorIndex_, RegisterIndex_, RegisterElementIndices_, OutFastMatrix_>
			(
				translations_
			)...
		);
	...
	[[nodiscard]] constexpr inline auto _make_translation_major_chunk(Translations_& translations_)
		-> typename EmuCore::TMP::remove_ref_cv_t<OutFastMatrix_>::major_chunk_type
	{
		...
		if constexpr (sizeof...(RegisterIndices_) > 1)
		{
			...
			({
				_make_translation_register<OutFastMatrix_, MajorIndex_, RegisterIndices_>
				(
					translations_,
		...
		{
			return _make_translation_register<OutFastMatrix_, MajorIndex_, 0>(translations_, per_register_element_sequence());
		}
	}
	...
	[[nodiscard]] constexpr inline auto _make_translation_matrix
	(
		Translations_& translations_,
	...
	) -> typename EmuCore::TMP::remove_ref_cv<OutFastMatrix_>::type
	{
		...
		(
			_make_translation_major_chunk<OutFastMatrix_, MajorIndices_, RegisterIndices_...>
			(
				translations_
			)...
		);
