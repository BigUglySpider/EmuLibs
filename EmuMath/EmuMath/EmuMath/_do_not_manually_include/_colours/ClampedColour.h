	...
	namespace Helpers
	{
		...
		struct do_colour_clamp
		{
			...
			constexpr inline TargetChannelType_ operator()(Val_ set_val_) const
			{
				...
				{
					return EmuMath::Helpers::clamp_colour_channel<TargetChannelType_, Val_>(set_val_);
				}
				else
				{
					return EmuMath::Helpers::clamp_colour_channel<TargetChannelType_, TargetChannelType_>(static_cast<TargetChannelType_>(set_val_));
				}
			}
		};
	}
