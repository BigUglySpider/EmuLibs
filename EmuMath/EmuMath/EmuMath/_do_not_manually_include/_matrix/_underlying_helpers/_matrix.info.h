...
namespace EmuMath::TMP
{
	...
	struct common_matrix_info
	{
		...
		// Major-order info
		// --- # of major elements = size in opposite direction (e.g. if column major, each column stores num_rows entries).
		static constexpr std::size_t num_major_elements = is_column_major ? num_rows : num_columns;
		static constexpr std::size_t num_non_major_elements = is_column_major ? num_columns : num_rows;

		// Matrix storage types and their info
