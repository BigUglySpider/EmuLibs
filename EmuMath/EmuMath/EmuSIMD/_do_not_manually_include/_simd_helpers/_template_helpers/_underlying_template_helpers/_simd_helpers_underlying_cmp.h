...
#pragma region MOVEMASK_EXTRACTION
	template<std::size_t PerElementWidthIfGenericInt_, class Register_>
	[[nodiscard]] inline auto _movemask(Register_&& register_)
	{
		...
		if constexpr (EmuSIMD::TMP::is_simd_register_v<register_type_uq>)
		{
			...
			else if constexpr (std::is_same_v<register_type_uq, EmuSIMD::i128_generic>)
			{
				...
				{
					static_assert(EmuCore::TMP::get_false<std::size_t, PerElementWidthIfGenericInt_>(), "Attempted to acquire a movemask from a 128-bit generic integral register via EmuSIMD helpers, but the passed per-element width is invalid.");
				}
			}
			else if constexpr (std::is_same_v<register_type_uq, EmuSIMD::i256_generic>)
			{
				...
				{
					static_assert(EmuCore::TMP::get_false<std::size_t, PerElementWidthIfGenericInt_>(), "Attempted to acquire a movemask from a 256-bit generic integral register via EmuSIMD helpers, but the passed per-element width is invalid.");
				}
			}
			else if constexpr (std::is_same_v<register_type_uq, EmuSIMD::i512_generic>)
			{
				...
				{
					static_assert(EmuCore::TMP::get_false<std::size_t, PerElementWidthIfGenericInt_>(), "Attempted to acquire a movemask from a 512-bit generic integral register via EmuSIMD helpers, but the passed per-element width is invalid.");
				}
			}
			...
			{
				static_assert(EmuCore::TMP::get_false<Register_>(), "Attempted to acquire a movemask via EmuSIMD helpers, but the passed SIMD register type is not supported for this operation.");
			}
		}
		else
		{
