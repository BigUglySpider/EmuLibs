...
#pragma region SHUFFLE_TEMPLATES
	...
	{
		return _mm256_shuffle_ps(lhs_, rhs_, ShuffleMask_);
	}

	template<shuffle_mask_type ShuffleMask_>
	EMU_SIMD_COMMON_FUNC_SPEC EmuSIMD::f32x8 permute_f32x8(EmuSIMD::f32x8_arg in_)
	{
		return _mm256_permute_ps(in_, ShuffleMask_);
	}
#pragma endregion
...
#pragma region BLEND_TEMPLATES
	...
	{
		return _mm256_blend_ps(a_, b_, BlendMask_);
	}
#pragma endregion
...
#else
			return EmuSIMD::_underlying_impl::emulate_extraction<f32x8, 256, Index_>(a_);
#endif
		}
	}
...
#pragma region GET_TEMPLATES
		...
		-> typename std::remove_cvref<OutT_>::type
	{
		if constexpr (Index_ == 0)
		{
			return static_cast<typename std::remove_cvref<OutT_>::type>(get_first_f32x8(in_));
		}
		else
		{
			if constexpr (Index_ <= 7)
			{
				...
				constexpr std::size_t is_hi = static_cast<std::size_t>(Index_ >= 4);
				constexpr std::size_t index_in_half = Index_ - (4 * is_hi);
				return EmuSIMD::Funcs::extract_element_f32x4<index_in_half, OutT_>(EmuSIMD::Funcs::extract_f32x8_lane_f32x4<is_hi>(in_));
#else
