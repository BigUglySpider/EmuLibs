		...
		if constexpr (EmuSIMD::TMP::is_simd_register_v<register_type_uq>)
		{
			...
			else
			{
				...
				if constexpr (EmuCore::TMP::is_any_same_v<register_type_uq, EmuSIMD::i128_generic, EmuSIMD::i256_generic, EmuSIMD::i512_generic>)
				{
					...
					{
						static_assert(EmuCore::TMP::get_false<Register_>(), "NO SUPPORT ERROR: EmuSIMD does not provide support for shuffling 16-bit-element SIMD registers. As 8 index arguments have been provided with a generic 128-bit integral register, elements have been assumed as 16-bit.");
					}
