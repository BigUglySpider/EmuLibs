...
#include "_noise_info/_noise_permutation_info.h"
...
namespace EmuMath
{
	...
	public:
		...
		static constexpr inline EmuMath::Vector<num_dimensions, OutT_> make_correctly_sized_vector(X_ x_, Y_ y_, Z_ z_)
		{
			if constexpr (num_dimensions == 1)
			{
				return EmuMath::Vector<1, OutT_>(x_);
			}
			else if constexpr (num_dimensions == 2)
			{
				return EmuMath::Vector<2, OutT_>(x_, y_);
			}
			else if constexpr (num_dimensions == 3)
			{
				return EmuMath::Vector<3, OutT_>(x_, y_, z_);
			}
			else
			{
		...
		static constexpr EmuMath::Vector<num_dimensions, value_type> _default_start_point = make_correctly_sized_vector<value_type>(0, 0, 0);
		static constexpr EmuMath::Vector<num_dimensions, value_type> _default_end_point = make_correctly_sized_vector<value_type>(1, 1, 1);
		static constexpr EmuMath::Vector<num_dimensions, std::size_t> _default_table_resolution = make_correctly_sized_vector<std::size_t>(32, 32, 32);
