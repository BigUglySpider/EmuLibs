...
#pragma region MINMAX_FUNCS
	...
	template<typename Out_ = std::uint32_t>
...
#else
			return EmuSIMD::_underlying_impl::emulate_extraction<u32x4, 128, Index_>(a_);
#endif
		}
	}
...
#pragma region GET_TEMPLATES
	...
	EMU_SIMD_COMMON_FUNC_SPEC auto extract_element_u32x4(u32x4_arg in_)
		-> typename std::remove_cvref<OutT_>::type
	{
		if constexpr (Index_ == 0)
		{
			return static_cast<typename std::remove_cvref<OutT_>::type>(EmuSIMD::Funcs::get_first_u32x4(in_));
		}
		else
		{
			...
			else
			{
				...
				(
					...
					"Invalid index provided when extracting an element from a u32x4 instance. Valid indices are 0, 1, 2, 3."
				);
			}
		}
	}
